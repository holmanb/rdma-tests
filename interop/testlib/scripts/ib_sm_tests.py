...
def nodePairs(node1, node2, guid_list):
    ...
    node1_guid = re.search( r"Node GUID.*0x(.*)", node1_ibstat_output[0])[1].strip
    node1_lid = re.search( r"SM lid: (.*)", node1_ibstat_output[0])[1].strip

    print(node1_guid)
    print(node1_lid)

    node2_ibstat_output = node2.command("sudo ibstat")
    node2_guid = re.search( r"Node GUID.*0x(.*)", node2_ibstat_output[0])[1].strip
    node2_lid = re.search( r"SM lid:(.*)", node2_ibstat_output[0])[1].strip
